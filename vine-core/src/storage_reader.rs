                    ...
                    if let Ok(row) = row_result {
                        ...
                        for field in fields {
                            ...
                            let value = match data_type {
                                ...
                                "boolean" => row.get_bool(col_index).unwrap_or_default().to_string(),
                                "double" => row.get_double(col_index).unwrap_or_default().to_string(),
                                _ => {
                                    eprintln!("Warning: Unsupported data type '{}' for field '{}', using empty string",
                                             data_type, field["name"].as_str().unwrap_or("unknown"));
                                    String::from("")
                                }
                            };
                            values.push(value);

...
use crate::vortex_exp::read_vortex_file;

/// Read vine data as Vortex arrays
///
/// # Arguments
/// * `dir_path` - Base directory containing date-partitioned Vortex files
///
/// # Returns
/// Vector of Vortex ArrayRef
pub fn read_vine_data(dir_path: &str) -> Vec<VortexArrayRef> {
...
fn read_vine_data_internal(dir_path: &str) -> Result<Vec<VortexArrayRef>, Box<dyn std::error::Error>> {
    let metadata = global_cache::get_reader_metadata(dir_path)?;
    read_with_metadata(dir_path, &metadata)
}

/// Read data with provided metadata
fn read_with_metadata(dir_path: &str, _metadata: &Metadata) -> Result<Vec<VortexArrayRef>, Box<dyn std::error::Error>> {
    let base_path = PathBuf::from(dir_path);

    let mut all_arrays: Vec<VortexArrayRef> = Vec::new();
